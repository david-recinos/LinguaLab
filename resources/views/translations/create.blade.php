    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100" x-data="{ type: '{{ old('type', 'word') }}' }">
                    <form method="POST" action="{{ route('translations.store') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="target_language_id" id="target_language_id"
                                ...
                                @foreach($targetLanguages as $tl)
                                    <option value="{{ $tl->target_language_id }}" {{ old('target_language_id') == $tl->target_language_id ? 'selected' : '' }}>
                        ...
                        <div class="mb-4" x-show="type === 'word'" x-transition>
                            ...
                            <select name="word_type_id" id="word_type_id"
                                ...
                                @foreach($wordTypes as $wordType)
                                    <option value="{{ $wordType->id }}" {{ old('word_type_id') == $wordType->id ? 'selected' : '' }}>
                                        {{ $wordType->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="target_text" id="target_text" rows="2"
                                      class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                      required>{{ old('target_text') }}</textarea>
                            @error('target_text')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="pronunciation" id="pronunciation" value="{{ old('pronunciation') }}"
                                   class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            @error('pronunciation')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="example_sentence" id="example_sentence" rows="2"
                                      class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('example_sentence') }}</textarea>
